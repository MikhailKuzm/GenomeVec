...
try:
    ...
    if (_SCRIPT_DIR / "embedders" / "DNABERT2").exists():
        _SCRIPT_DIR = _SCRIPT_DIR / "embedders" / "DNABERT2"
...
def _ensure_triton_stub():
    """На платформах без Triton (например Windows) подменяем модуль stub'ом, чтобы импорт
    flash_attn_triton падал и bert_layers использовал fallback. На Linux оставляем настоящий triton."""
...
def _patch_flash_attn_triton_trans_b():
    """Патчит кэшированный flash_attn_triton.py: tl.dot(..., trans_b=True) не поддерживается
    в новых Triton — заменяем на tl.dot(a, tl.trans(b))."""
    import os
    import re
    candidates = [
    ...
    for c in candidates:
        if c is None or not c.is_dir():
            continue
        base = c / "modules" / "transformers_modules"
        if not base.is_dir():
            continue
        zhihan = base / "zhihan1996"
        if not zhihan.is_dir():
            continue
        for repo_dir in zhihan.iterdir():
            if not repo_dir.is_dir():
                continue
            for rev_dir in repo_dir.iterdir():
                if not rev_dir.is_dir():
                    continue
                fa = rev_dir / "flash_attn_triton.py"
                if not fa.is_file():
                    continue
                try:
                ...
                if new_text != text:
                    try:
                        fa.write_text(new_text, encoding="utf-8")
                    except Exception:
                        pass
...
def _load_model_and_tokenizer(model_path: str | Path | None = None, device: str | None = None):
    ...
    _ensure_triton_stub()
    _patch_flash_attn_triton_trans_b()
...
def embeddings_matrix_for_windows(
    tokens_per_contig: list[list[int]],
    model=None,
    tokenizer=None,
    model_path: str | Path | None = None,
    ...
    """
    Строит матрицу эмбеддингов: по одному вектору на каждое окно, в порядке следования окон
    сверху вниз по контигам (как в исходном FASTA).

    Returns:
        (matrix, meta_info): matrix — тензор (num_windows, hidden_size); meta_info — num_windows, time_seconds, num_contigs.
    """
    if not tokens_per_contig:
        raise ValueError("tokens_per_contig пустой")
    if model is None or tokenizer is None:
    ...
    for contig_tokens in tokens_per_contig:
        ...
        while start + window_size <= len(contig_tokens):
            ...
            start += stride
        if start < len(contig_tokens) and len(contig_tokens) - start >= 1:
            win = contig_tokens[-window_size:]
            windows_to_process.append(win)

    if not windows_to_process:
        raise ValueError("Не получилось ни одного окна")

    num_windows = len(windows_to_process)
    matrix = torch.zeros(num_windows, hidden_size, dtype=torch.float32, device=device)
    start_time = time.perf_counter()

    for i in tqdm(range(0, num_windows, batch_size), desc="Windows", unit="batch"):
        batch = windows_to_process[i : i + batch_size]
        input_ids = torch.tensor([w for w in batch], dtype=torch.long, device=device)
        attention_mask = torch.ones_like(input_ids, device=device)
        with torch.no_grad():
